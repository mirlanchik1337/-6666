...
async def fsm_start(message: types.Message):
    if message.chat.type == "private":
        await FSMAdmin.name.set()  # Обращяемся к классу FSMAdmin и вызываем у него метод состояния name
...
async def load_gender(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    await message.answer('Откуда ты?', reply_markup=cancel_markup)
...
async def load_photo(message: types.Message, state: FSMContext):
    ...
    #     await message.answer("Ошибка: отправьте фотографию")
    #     return
    # else:
    # условие не работает
        async with state.proxy() as data:
